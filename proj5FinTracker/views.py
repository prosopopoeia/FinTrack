...
def index(request):
    if request.user.is_authenticated:
        return render(request, "proj5FinTracker/month.html")
...
# @login_required
# @csrf_exempt
# def jstotals(request):

    # data = json.loads(request.body)
    # vmonth = data["jsmonth"]
    # vyear = data["jsyear"]
    # user = get_user(request);
    # sum_debit  = BankTransaction.objects.filter(trans_date__year=vyear, trans_date__month=vmonth, trans_amt__lt=0).aggregate(Sum('trans_amt'))
    # sum_credit = BankTransaction.objects.filter(trans_date__year=vyear, trans_date__month=vmonth, trans_amt__gt=0).aggregate(Sum('trans_amt'))
    
    # return JsonResponse({"sum_cr": sum_credit, "sum_db": sum_debit}, status=201) 
